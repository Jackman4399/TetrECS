...
public class GameBoard extends GridPane {
  ...
  /**
   * The listener for when a there is a right click
   */
  private RightClickedListener rightClickedListener;

  /**
   * A boolean for the keyboard
   */
  ...
  protected GameBlock createBlock(int x, int y) {
      ...
      if(!(this instanceof PieceBoard)) {
        block.setOnMouseEntered((event -> block.hover(true)));
        block.setOnMouseExited((event -> block.hover(false)));
      }
  ...
  /**
   * Set the listener to handle an event when the main game board is clicked
   *
   * @param listener listener to add
   */
  ...
  /**
   * Triggered when a block is left-clicked. Call the attached listener.
   *
   ...
   * @param event mouse event
   * @param gb the game board clicked
   */
  private void rightClicked(MouseEvent event, GameBoard gb) {
    if (event.getButton() == MouseButton.SECONDARY) {
      logger.info("Right clicked main board.");
      if (rightClickedListener != null) {
        rightClickedListener.rightClicked(gb, true);
      }
    } else if (event.getButton() == MouseButton.PRIMARY){
      logger.info("Left clicked current board.");
      if (rightClickedListener != null) {
        rightClickedListener.rightClicked(gb, false);
      }
    }
  }
  ...
  public void fadeOut(HashSet<GameBlockCoordinate> coordinates){
    ...
    while(iterator.hasNext()){
      ...
      var gbc = iterator.next();
      this.getBlock(gbc.getX(), gbc.getY()).fade();
    }
  }
